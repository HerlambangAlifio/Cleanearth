...
const eventGambarInput = document.getElementById('eventGambar');
const previewEventGambar = document.getElementById('previewEventGambar');

function showEventPreviewFromFile(file) {
  if (!file) {
    previewEventGambar.innerHTML = '';
    previewEventGambar.style.display = 'none';
    return;
  }
  if (!file.type.startsWith('image/')) {
    previewEventGambar.innerHTML = '<span style="color:#dc2626;">File bukan gambar!</span>';
    previewEventGambar.style.display = 'block';
    return;
  }
  const reader = new FileReader();
  reader.onload = function(ev) {
    previewEventGambar.innerHTML = `<img src="${ev.target.result}" style="max-width:180px;max-height:120px;border-radius:8px;">`;
    previewEventGambar.style.display = 'block';
  };
  reader.readAsDataURL(file);
}

if (eventGambarInput && previewEventGambar) {
  eventGambarInput.addEventListener('change', function(e) {
    showEventPreviewFromFile(e.target.files[0]);
  });
}
...
window.editEvent = function(id, judul, deskripsi, lokasi, tanggal, waktuMulai, waktuSelesai, kapasitas, status, gambar) {
  ...
  // Tampilkan preview gambar event lama jika ada
  if (previewEventGambar) {
    if (gambar) {
      previewEventGambar.innerHTML = `<img src='http://localhost:5000/uploads/${gambar}' style='max-width:180px;max-height:120px;border-radius:8px;'>`;
      previewEventGambar.style.display = 'block';
    } else {
      previewEventGambar.innerHTML = '';
      previewEventGambar.style.display = 'none';
  ...
  // Reset input file (agar user bisa upload file baru jika mau)
  if (eventGambarInput) eventGambarInput.value = '';
};
